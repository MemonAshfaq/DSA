...
Input: str s
Output: int (length of the longest substring with all unique chars)
...
- All identical characters (answer is 1 if s not empty)
- Repeats that force window to jump past last seen index (e.g., "abba", "tmmzuxt")

Pattern: Sliding Window + Last-Index Map
"""

def longest_substring_no_repeat_bf_on3(s: str) -> int:
    l = len(s)
    ml = 0

    for i in range(0,l):          # (1) outer loop → runs n times
        ss = s[i]                 # (2) start substring (string copy, O(1) here since 1 char)
        for j in range(i+1,l):    # (3) inner loop → up to n times per i
            if s[j] in ss:        # (4) membership test → O(len(ss)) = O(n) worst case
                break
            ss = s[i:j+1]         # (5) slice copy → O(j-i) = O(n) worst case
        ml = max(ml, len(ss))

    return ml

def longest_substring_no_repeat_bf_on2(s: str) -> int:
    """
    Brute force: check all substrings and verify uniqueness with a set.
    For each (i, j), ensure s[i:j+1] has no repeated characters.
    T: O(n^3)  (O(n^2) substrings * O(n) uniqueness check)
       (can be improved to O(n^2) if we build the set incrementally per i)
    S: O(min(n, Σ)) for the set (Σ = charset size)
    """
    l = len(s)
    ml = 0
    for i in range(0,l):    # tc : o(n)
        seen = set()        # sc = o(1)
        seen.add(s[i])      # sc = o(k) # k = unique
        for j in range(i+1,l): # tc: o(n)
                if s[j] in seen:  #tc: o(1) # set membership check (hash table)
                     break
                seen.add(s[j]) 
        ml = max(ml, len(seen))
...
def longest_substring_no_repeat_op_on2(s):
    l = 0
    best = 0
    last = {}
    window = ""
...
def longest_substring_no_repeat_op_on1(s: str) -> int:
    """
    Optimal (Sliding Window + last index map):
    Expand right pointer; track last seen index of each char.
    When a repeat appears, move left pointer to max(left, last_index[ch]+1).
    Keep best window size as we go.
    T: O(n) — each index moves at most once
    S: O(min(n, Σ)) for the map
    """
    last = {}
    left = 0
    best = 0
...
---------------------------------|--------|-------|-------------------------------
longest_substring_no_repeat_bf   | O(n^3) | O(n)  | Check every substring; set per check
longest_substring_no_repeat_op   | O(n)   | O(n)  | Sliding window + last index map
------------------------------------------------------------
"""
def _demo_longest_substring_no_repeat():
    #---------------- longest_substring_no_repeat_bf o(n3)-----------------
    CHECK('BF: empty -> 0',          longest_substring_no_repeat_bf_on3("") == 0)
    CHECK('BF: "a" -> 1',            longest_substring_no_repeat_bf_on3("a") == 1)
    CHECK('BF: "bbbbb" -> 1',        longest_substring_no_repeat_bf_on3("bbbbb") == 1)
    CHECK('BF: "abcabcbb" -> 3',     longest_substring_no_repeat_bf_on3("abcabcbb") == 3)
    CHECK('BF: "pwwkew" -> 3',       longest_substring_no_repeat_bf_on3("pwwkew") == 3)
    CHECK('BF: "dvdf" -> 3',         longest_substring_no_repeat_bf_on3("dvdf") == 3)
    CHECK('BF: "abba" -> 2',         longest_substring_no_repeat_bf_on3("abba") == 2)
    CHECK('BF: "tmmzuxt" -> 5',      longest_substring_no_repeat_bf_on3("tmmzuxt") == 5)  # "mzuxt"

    #---------------- longest_substring_no_repeat_bf o(n2)-----------------
    CHECK('BF: empty -> 0',          longest_substring_no_repeat_bf_on2("") == 0)
    CHECK('BF: "a" -> 1',            longest_substring_no_repeat_bf_on2("a") == 1)
    CHECK('BF: "bbbbb" -> 1',        longest_substring_no_repeat_bf_on2("bbbbb") == 1)
    CHECK('BF: "abcabcbb" -> 3',     longest_substring_no_repeat_bf_on2("abcabcbb") == 3)
    CHECK('BF: "pwwkew" -> 3',       longest_substring_no_repeat_bf_on2("pwwkew") == 3)
    CHECK('BF: "dvdf" -> 3',         longest_substring_no_repeat_bf_on2("dvdf") == 3)
    CHECK('BF: "abba" -> 2',         longest_substring_no_repeat_bf_on2("abba") == 2)
    CHECK('BF: "tmmzuxt" -> 5',      longest_substring_no_repeat_bf_on2("tmmzuxt") == 5)  # "mzuxt"

    #---------------- longest_substring_no_repeat_op -----------------
    CHECK('OP: empty -> 0',          longest_substring_no_repeat_op_on1("") == 0)
    CHECK('OP: "a" -> 1',            longest_substring_no_repeat_op_on1("a") == 1)
    CHECK('OP: "bbbbb" -> 1',        longest_substring_no_repeat_op_on1("bbbbb") == 1)
    CHECK('OP: "abcabcbb" -> 3',     longest_substring_no_repeat_op_on1("abcabcbb") == 3)
    CHECK('OP: "pwwkew" -> 3',       longest_substring_no_repeat_op_on1("pwwkew") == 3)
    CHECK('OP: "dvdf" -> 3',         longest_substring_no_repeat_op_on1("dvdf") == 3)
    CHECK('OP: "abba" -> 2',         longest_substring_no_repeat_op_on1("abba") == 2)
    CHECK('OP: "tmmzuxt" -> 5',      longest_substring_no_repeat_op_on1("tmmzuxt") == 5)

    #---------------- longest_substring_no_repeat_op -----------------
    CHECK('OP: empty -> 0',          longest_substring_no_repeat_op_on2("") == 0)
    CHECK('OP: "a" -> 1',            longest_substring_no_repeat_op_on2("a") == 1)
    CHECK('OP: "bbbbb" -> 1',        longest_substring_no_repeat_op_on2("bbbbb") == 1)
    CHECK('OP: "abcabcbb" -> 3',     longest_substring_no_repeat_op_on2("abcabcbb") == 3)
    CHECK('OP: "pwwkew" -> 3',       longest_substring_no_repeat_op_on2("pwwkew") == 3)
...
def demo_lc_sliding_window():
    ...
    _demo_longest_substring_no_repeat()
